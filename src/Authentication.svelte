<script>
    ...
    function setupInputFocusEvent() {
        ...
        for(let i = 0; i < inputs.length; i++) {
            inputs[i].addEventListener('focus',(event) => {
                ...
                if(errorMessage === "Fill in all the fields")
                    errorMessage = null
	...
	function sendPacket() {
		...
		if(canSend)
			client.send(JSON.stringify(authenticationPacket))
	}
...
<main>
    <div id="grid-container">
        ...
        <div id="form-area" style="{authenticationState == LOGIN ? "height:22rem !important;" : ""}">
            <div id="form">
               <div id="form-inputs">
                {#if accountCreated}
                <div> <span id="account-created">Account has been created, you can login now</span> </div>
                {/if}
               {#if authenticationState == REGISTER}
               <div class="form-input">
                <input bind:value={authenticationPacket.email} type="email" placeholder="Email" />
                </div>
               {:else}
               ...
               {/if}
                <div class="form-input">
                    <input bind:value={authenticationPacket.username} type="text" placeholder="Username" />
                </div>
                <div class="form-input">
                    <input bind:value={authenticationPacket.password} type="password" placeholder="Password" />
                </div>
               </div>
                {#if errorMessage}
                <br>
                ...
                {#if authenticationState == REGISTER}
                <br>
                    <span>Already registered an account? <button on:click={switchUserInterface} id="swap-ui-btn">login</button> here</span>
                <br>
                <button on:click={submitForm} {disabled} id="submit-ui-btn">Register</button>
                {:else}
                 <br>
                    <span>Don't have an account? <button on:click={switchUserInterface} id="swap-ui-btn">register</button> here</span>
                  <br>
                <button on:click={submitForm} {disabled} id="submit-ui-btn">Login</button>
                {/if}
            </div>
        </div>
    </div>
